...
# 加载音效
bubble_sound = pygame.mixer.Sound('音效/按鍵聲屋薩其.wav')  # 替换为你的點擊音效路径
hover_sound = pygame.mixer.Sound('音效/按鍵聲氣泡音.wav')  # 替换为你的懸停音效路径
correct_sound = pygame.mixer.Sound('音效/正確音效.wav')  # 替换为你的正確答案音效路径
wrong_sound = pygame.mixer.Sound('音效/錯誤音效.wav')  # 替换为你的錯誤答案音效路径
...
def render_question_and_options(screen, question_data, title_font, option_font, selected_option, hovered_option):
    """渲染问题和选项，带有悬停和点击效果"""
    screen.blit(config.qa_background, (0, 0))  # 背景图像
...
def questionRun(screen, questions_data, title_font, option_font):
    ...
    while running:
        ...
        # 渲染問題和選項
        render_question_and_options(screen, questions_data[current_question_index], title_font, option_font, selected_option, hovered_option)
